...
- (void)fakeLoadReady {
  ...
  if (_type & MockPhotoSourceLoadError) {
      objc_msgSend(_delegates, @selector(model:didFailLoadWithError:), self, nil);
//    [_delegates performSelector: @selector(model:didFailLoadWithError:)
//                                withObject: self
//                                withObject: nil];
  } else {
    ...
    [_delegates performSelector:@selector(modelDidFinishLoad:) withObject:self];
  }
}
...
- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more {
  if (cachePolicy & TTURLRequestCachePolicyNetwork) {
    [_delegates performSelector:@selector(modelDidStartLoad:) withObject:self];
